    ...
    return product && (
        <div className="mt-12">
            ...
            <div className="flex flex-col items-center mt-20">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mt:gap-6 lg:grid-cols-5 mt-6 w-full">
                              {relatedProducts.filter((product)=>product.inStock).map
                              ((product,index)=>(
                                        <ProductCard key={index} product = {product}/>
                              ))}
                    </div>
                    <button onClick={()=>{navigate('/products');scrollTo()}} className="mx-auto cursor-pointer px-12 my-16 py-2.5
                    border rounded text-primary hover:bg-primary/10">See More</button>
            </div>
        </div>
    );
};
